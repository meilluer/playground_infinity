package com.meilluer.infinity.network
...
class AnonymousAccessTokenInterceptor(
    ...
    private fun getOrFetchToken(): String? {
        return cachedAccessToken.get() ?: synchronized(refreshLock) {
            val accessToken = redditDataRoomDatabase.accountDao().anonymousAccessToken
    ...
    private fun refreshToken(): String? {
        return try {
            ...
            if (response.isSuccessful) {
                ...
                redditDataRoomDatabase.accountDao().setAnonymousAccessToken(newAccessToken)
