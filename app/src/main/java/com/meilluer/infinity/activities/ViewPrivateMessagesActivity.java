...
import com.meilluer.infinity.adapters.PrivateMessagesDetailRecyclerViewAdapter;
...
import com.meilluer.infinity.databinding.ActivityViewPrivateMessagesBinding;
import com.meilluer.infinity.events.PassPrivateMessageEvent;
import com.meilluer.infinity.events.PassPrivateMessageIndexEvent;
import com.meilluer.infinity.events.RepliedToPrivateMessageEvent;
import com.meilluer.infinity.message.Message;
import com.meilluer.infinity.message.ReadMessage;
import com.meilluer.infinity.message.ReplyMessage;
...
public class ViewPrivateMessagesActivity extends BaseActivity implements ActivityToolbarInterface {
                ...
                if (!binding.editTextViewPrivateMessagesActivity.getText().toString().equals("")) {
                    ...
                    if (privateMessage != null) {
                        ...
                        if (fullnames.length() > 0) {
                            String fullname = fullnames.substring(0, fullnames.length() - 1);
                            int inboxCount = mCurrentAccountSharedPreferences.getInt(com.meilluer.infinity.utils.SharedPreferencesUtils.INBOX_COUNT, 0);
                            inboxCount -= fullname.split(",").length;
                            if (inboxCount < 0) {
                                inboxCount = 0;
                            }
                            mCurrentAccountSharedPreferences.edit().putInt(com.meilluer.infinity.utils.SharedPreferencesUtils.INBOX_COUNT, inboxCount).apply();
                            EventBus.getDefault().post(new com.meilluer.infinity.events.ChangeInboxCountEvent(inboxCount));
                            ReadMessage.readMessage(mOauthRetrofit, accessToken, fullname,
    ...
    public void onPassPrivateMessageEvent(PassPrivateMessageEvent passPrivateMessageEvent) {
        ...
        if (privateMessage != null) {
            ...
            if (privateMessage.getReplies() != null && !privateMessage.getReplies().isEmpty()) {
                for (Message m : privateMessage.getReplies()) {
                    if (m.isNew()) {
