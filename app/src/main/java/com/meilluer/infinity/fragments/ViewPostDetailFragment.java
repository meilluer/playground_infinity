...
import static com.meilluer.infinity.activities.CommentActivity.RETURN_EXTRA_COMMENT_DATA_KEY;
import static com.meilluer.infinity.activities.CommentActivity.WRITE_COMMENT_REQUEST_CODE;
import static com.meilluer.infinity.videoautoplay.media.PlaybackInfo.INDEX_UNSET;
import static com.meilluer.infinity.videoautoplay.media.PlaybackInfo.TIME_UNSET;
...
import com.meilluer.infinity.PostModerationActionHandler;
...
import com.meilluer.infinity.adapters.CommentsRecyclerViewAdapter;
import com.meilluer.infinity.adapters.PostDetailRecyclerViewAdapter;
import com.meilluer.infinity.apis.RedditAPI;
import com.meilluer.infinity.apis.StreamableAPI;
import com.meilluer.infinity.archive.ArcticShiftRestorer;
import com.meilluer.infinity.bottomsheetfragments.FlairBottomSheetFragment;
import com.meilluer.infinity.bottomsheetfragments.PostCommentSortTypeBottomSheetFragment;
import com.meilluer.infinity.comment.Comment;
import com.meilluer.infinity.comment.FetchComment;
import com.meilluer.infinity.comment.ParseComment;
import com.meilluer.infinity.commentfilter.CommentFilter;
import com.meilluer.infinity.commentfilter.FetchCommentFilter;
...
import com.meilluer.infinity.databinding.FragmentViewPostDetailBinding;
import com.meilluer.infinity.events.ChangeNSFWBlurEvent;
import com.meilluer.infinity.events.ChangeNetworkStatusEvent;
import com.meilluer.infinity.events.ChangeSpoilerBlurEvent;
...
import com.meilluer.infinity.message.ReadMessage;
import com.meilluer.infinity.post.FetchPost;
import com.meilluer.infinity.post.HidePost;
import com.meilluer.infinity.post.ParsePost;
import com.meilluer.infinity.post.Post;
import com.meilluer.infinity.readpost.InsertReadPost;
import com.meilluer.infinity.readpost.ReadPostsUtils;
...
import com.meilluer.infinity.thing.DeleteThing;
...
import com.meilluer.infinity.videoautoplay.media.VolumeInfo;
import com.meilluer.infinity.viewmodels.ViewPostDetailActivityViewModel;
import com.meilluer.infinity.viewmodels.ViewPostDetailFragmentViewModel;
...
public class ViewPostDetailFragment extends Fragment implements FragmentCommunicator, PostModerationActionHandler, PostDetailRecyclerViewAdapter.OnLongClickTtsListener, PostDetailRecyclerViewAdapter.OnTtsClickListener {
    ...
    private void restoreScrollPosition() {
        ...
        if (mPost != null && activity != null) {
            mExecutor.execute(() -> {
                com.meilluer.infinity.readpost.ReadComment readComment = mRedditDataRoomDatabase.readCommentDao().getReadComment(activity.accountName, mPost.getId());
        ...
        if (mCommentsAdapter != null && mPost != null && activity != null) {
            ...
            if (position >= 0) {
                ...
                if (commentIndex > 0) {
                    ...
                    mExecutor.execute(() -> {
                        try {
                            com.meilluer.infinity.readpost.ReadComment readComment = new com.meilluer.infinity.readpost.ReadComment(
    ...
    private void fetchComments(boolean changeRefreshState, SortType.Type sortType) {
        ...
        if (getArguments() != null && getArguments().getBoolean(EXTRA_IS_OFFLINE_POST, false)) {
            mExecutor.execute(() -> {
                List<com.meilluer.infinity.offline.OfflineComment> offlineComments = mRedditDataRoomDatabase.offlineCommentDao().getOfflineCommentsByPostId(mPost.getId());
    ...
    private com.meilluer.infinity.utils.TtsManager mSequentialTtsManager;
    ...
    public void onTtsClick(Comment comment, View itemView) {
        ...
        if (mSequentialTtsManager == null) {
            mSequentialTtsManager = new com.meilluer.infinity.utils.TtsManager(activity);
        }
        mIsReadingAll = false;
        mSequentialTtsManager.speak(comment.getCommentRawText(), () -> {
            unhighlightCurrentSentence();
        }, new com.meilluer.infinity.utils.TtsManager.OnTtsUpdateListener() {
            @Override
    ...
    public void onTtsClick(Post post) {
        ...
        mSequentialTtsManager.speak(textToSpeak, () -> {
            unhighlightCurrentSentence();
        }, new com.meilluer.infinity.utils.TtsManager.OnTtsUpdateListener() {            @Override
            public void onSentenceStart(String text, int start, int end) {
                        ...
                        if (sentenceIndex >= 0) {
                            ...
                            if (wordStartIndex >= 0 && wordEndIndex <= contentString.length()) {
                                ...
                                if (mHighlightSpan == null) {
                                    mHighlightSpan = new android.text.style.BackgroundColorSpan(mCustomThemeWrapper.getColorAccent());
                                }
